...
-- Define a function to load a sound from a file
loadSoundFromFile :: FilePath -> IO Sound
loadSoundFromFile filePath = do
  -- Read the file and decode it to a Sound
  contents <- BS.readFile filePath
  case decodeWav contents of
    Left err -> error $ "Error decoding sound: " ++ err
    Right wav -> return $ WavSound wav
...
updateRound dt gameState
  ...
  where
    ...
    keysDown = [KeyLeft | elem 'q' keys] ++ [KeyLeft | "left" `elem` keys] ++ [KeyRight | elem 'd' keys] ++ [KeyRight | "right" `elem` keys]
...
main = do
  let initialState = GameState
        ...
        , soundBrick = loadSoundFromFile pathSoundBrick    -- Load your sound here
        , soundRacket = loadSoundFromFile pathSoundRacket  -- Load your sound here
